...
cbsa_name <- read_csv('~/Documents/zillow/thesis/data/external_data/nhgis/nhgis0003_ds244_20195_2019_cbsa.csv') %>%
...
cbsa_2003 <- read_csv('~/Documents/zillow/thesis/data/external_data/CBSA_2003.csv') %>%
  separate(MsaName, c("Metro", "end"), ', ', remove = FALSE) %>%
  mutate(Metro = sub("-.*", "", Metro),
         MetroState = sub(" .*", "", end),
         MetroState = sub("-.*", "", MetroState),
...
pop <- read_csv('~/Documents/zillow/thesis/data/external_data/nhgis/nhgis0003_ds244_20195_2019_zcta.csv') %>%
...
wfh <- read_csv('~/Documents/zillow/thesis/data/lodes_rac_wfh_zip_exposure.csv') %>%
  distinct(zip, .keep_all = TRUE) %>%
  mutate(zip = as.integer(zip))
...
county_to_zip <- read_csv('~/Documents/zillow/thesis/data/external_data/county_zip_crosswalk.csv') %>%
...
zbp <- read_csv('~/Documents/zillow/thesis/data/zbp_wfh.csv') %>% 
...
write_csv(wfh_exp, '~/Documents/zillow/thesis/data/zip_all_chars.csv')
