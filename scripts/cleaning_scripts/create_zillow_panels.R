...
df6 <- df4 %>% select(zip, post_pct_change) %>% inner_join(df5, by = 'zip') %>%
  ...
  write_csv('~/Documents/zillow/thesis/data/zhvi_sfh_panel_zips.csv')

## Write to csv (top 12 metros)
df6 %>% filter(MetroShort %in% cities) %>%
  write_csv('~/Documents/zillow/thesis/data/zhvi_sfh_panel_zips_top12.csv')
...
msa_chars <- read_csv('~/Documents/zillow/thesis/data/msa_all_chars.csv') %>%
...
df5 <- df3 %>% left_join(df4, by = 'MetroShort') %>% left_join(msa_chars, by = 'MetroShort') %>% 
  ...
  write_csv('~/Documents/zillow/thesis/data/msa_zhvi.csv')
...
df6 <- df3 %>% select(zip, post_pct_change) %>% inner_join(df4, by = 'zip') %>%
  ...
  write_csv('zori_sfh_panel_zips.csv')

## Write panel to CSV (with top 12 metros)
df6 %>% filter(MetroShort %in% cities) %>%
  write_csv('~/Documents/zillow/thesis/data/zori_sfh_panel_zips_top12.csv')
...
df5 <- df3 %>% left_join(df4, by = 'MetroShort') %>% left_join(msa_chars, by = 'MetroShort') %>% 
  filter(!is.na(wfh_emp)) %>%
  write_csv('~/Documents/zillow/thesis/data/msa_zori.csv')
